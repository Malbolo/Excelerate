...
import { useGetLLMTemplate, useGetLLMTemplateByCategory, usePostCallPrompt } from '@/apis/playground';
...
const LLMPlaygroundPage = () => {
  ...
  const extractedVariableKeys = useMemo(() => {
    const allText = [systemPrompt, ...fewShots.flatMap(fs => [fs.human, fs.ai]), userInput].join(' ');
          ...
          <div className='space-y-5 p-4'>
            ...
            <div>
              <div className='mb-2.5 flex items-center justify-between'>
                <h3 className='text-xs font-medium text-gray-700'>Few-shot Prompts</h3>
                <Button
                  variant='outline'
                  size='sm'
                  onClick={addFewShot}
        ...
        <div className='flex w-2/5 flex-col gap-3 xl:w-1/3'>
          <Card className='flex-1'>
            ...
            <CardContent className='p-3 text-xs text-gray-600'>
              <p className='italic'>AI output based on initial parameters will appear here.</p>
            </CardContent>
          </Card>
          <Card className='flex-1'>
            <CardHeader className='p-3'>
              <CardTitle className='text-sm font-semibold text-gray-700'>Modified AI Output</CardTitle>
            </CardHeader>
            <CardContent className='p-3 text-xs text-gray-600'>
              <p className='italic'>AI output based on your modified prompts will appear here.</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <Dialog open={isVariablesDialogOpen} onOpenChange={setIsVariablesDialogOpen}>
        <DialogContent className='sm:max-w-[500px]'>
          <DialogHeader>
            ...
            <DialogDescription>
              Set the values for the variables found in your prompts. Click Done when you're finished.
            </DialogDescription>
          </DialogHeader>
            ...
            {extractedVariableKeys.length > 0 ? (
              extractedVariableKeys.map(key => (
                <div className='grid grid-cols-4 items-center gap-4' key={key}>
                  <Label htmlFor={`variable-dialog-${key}`} className='text-right'>
                    {key}
                  </Label>
                  <Input
              ...
              <p className='col-span-4 py-4 text-center text-sm text-gray-500'>No variables found in the prompts.</p>
            )}
          </div>
          <DialogFooter>
            <Button type='button' onClick={() => setIsVariablesDialogOpen(false)}>
              Done
            </Button>
          </DialogFooter>
        </DialogContent>
