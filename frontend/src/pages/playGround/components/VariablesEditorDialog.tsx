    ...
    >
      <DialogContent className='flex h-[70vh] max-w-lg flex-col p-0 sm:max-w-xl md:h-[75vh] md:max-w-2xl lg:max-w-3xl'>
        <DialogHeader className='p-6 pb-2'>
          <DialogTitle className='text-lg font-bold'>Edit Variables</DialogTitle>
          <DialogDescription>
        ...
        <div className='flex flex-1 gap-0 overflow-hidden border-t border-b'>
          <div className='w-1/3 overflow-y-auto border-r bg-slate-50/50 p-3'>
            <h4 className='sticky top-0 -mx-3 mb-2 border-b bg-slate-50/50 px-3 py-1.5 text-xs font-bold'>
              ...
              {currentVariableKeys.length > 0 ? (
                currentVariableKeys.map(key => (
                  <Button
                    ...
                    className={`hover:bg-secondary h-auto w-full justify-start truncate rounded-md px-2 py-1.5 text-left text-sm transition-colors duration-100 ease-in-out ${
                      selectedVariableKey === key ? 'text-accent-foreground bg-secondary font-medium' : 'text-slate-700'
                    }`}
                  >
                    {key}
                  </Button>
                ...
                <p className='px-1 py-2 text-xs italic'>No variables loaded.</p>
              )}
            </div>
          </div>
          ...
          <div className='flex w-2/3 flex-col space-y-2 bg-white p-4'>
            {selectedVariableKey && variables.hasOwnProperty(selectedVariableKey) ? (
              <>
                ...
                >
                  Editing: <span className='text-accent-foreground font-semibold'>{selectedVariableKey}</span>
                </Label>
                <Textarea
                  ...
                  className='flex-1 resize-none rounded-md border-slate-300 p-2.5 text-sm shadow-sm'
