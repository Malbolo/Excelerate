  ...
  return (
    <section className='flex min-w-1/2 flex-1 flex-col justify-between gap-2 pl-2'>
      <div className='flex items-baseline justify-between'>
        ...
        {sourceDataCommand && (
          <p className='ml-2 flex-1 truncate text-right text-xs text-gray-500'>{sourceDataCommand}</p>
        )}
      </div>
        ...
        {sourceParams && Object.keys(sourceParams).length > 0 ? (
          <div className='flex h-full flex-col'>
            <div className='sticky top-0 z-10 flex rounded-t-xl border-b bg-slate-100 font-bold'>
              ...
              <div className='flex-grow rounded-tr-xl px-2 py-2 text-xs font-semibold text-slate-700'>Value</div>
            </div>
              ...
              {Object.entries(sourceParams).map(([key, value], index) => (
                ...
                >
                  ...
                  <div className='flex-grow px-3 py-2 text-xs break-all text-slate-700'>
                    {typeof value === 'object' ? (
                      <Badge variant='destructive'>{JSON.stringify(value, null, 2)}</Badge>
                    ) : (
          ...
          <div className='flex flex-1 flex-col items-center justify-center p-4 text-center'>
            {!sourceDataCommand && (
              <>
                <p className='text-sm font-medium text-gray-700'>Source data hasn't been loaded.</p>
                <p className='mt-1 text-xs text-gray-500'>Please load it using a command.</p>
              </>
            )}
            {sourceDataCommand && (!sourceParams || Object.keys(sourceParams).length === 0) && (
