...
const ScheduleDetail = () => {
  ...
  return (
    <SchedulerMonitoringLayout title={scheduleData.title} backPath={`/scheduler-monitoring/day/${dayId}`}>
      <div className='mb-8 rounded-lg bg-white p-6 shadow'>
        <p className='mt-1 text-sm text-gray-500'>{scheduleData.title}</p>
        <div className='mt-2 text-xs text-gray-400'>
          <span>ID: {scheduleData.schedule_id} | </span>
          <span>Created: {new Date(scheduleData.start_time).toLocaleString()} | </span>
          <span>
            Status:
            <span className={`font-medium ${scheduleData.status === 'error' ? 'text-red-500' : 'text-green-500'}`}>
              {scheduleData.status}
            </span>
          </span>
        </div>
      ...
      <div className='grid grid-cols-2 gap-8 pb-8'>
        <div className='space-y-6'>
          <h2 className='mb-4 border-b pb-2 text-xl font-semibold text-gray-700'>Jobs</h2>
        ...
        <div className='space-y-6'>
          <h2 className='mb-4 border-b pb-2 text-xl font-semibold text-gray-700'>Error Logs</h2>
